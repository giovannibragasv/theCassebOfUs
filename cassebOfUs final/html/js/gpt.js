...
//portas fechadas
mapaFaseDois[14][29] = "D";
mapaFaseDois[5][2] = "D";
mapaFaseDois[24][2] = "D";
mapaFaseDois[5][27] = "D";
mapaFaseDois[24][23] = "D";
//chaves 
mapaFaseDois[23][1] = "@";
mapaFaseDois[26][1] = "@";
mapaFaseDois[4][15] = "@"; 
...
function geracaoDoMapa() {
    // definir o tamanho da circunferência
    const tamanhoCircunferencia = 30;
  
    // calcular os limites da circunferência
    const limiteSuperior = Math.max(0, linha - tamanhoCircunferencia);
    const limiteInferior = Math.min(tamanhoMapaFaseDois - 1, linha + tamanhoCircunferencia);
    const limiteEsquerdo = Math.max(0, coluna - tamanhoCircunferencia);
    const limiteDireito = Math.min(tamanhoMapaFaseDois - 1, coluna + tamanhoCircunferencia);
  
    // definir o espaço do mapa
    let espacoMapa = "";
  
    // gerar o mapa dentro dos limites da circunferência
    for (let x = limiteSuperior; x <= limiteInferior; x++) {
      espacoMapa += "\n"; // quebra de linha
  
      for (let y = limiteEsquerdo; y <= limiteDireito; y++) {
        espacoMapa += " "; // espaço entre personagens
  
        // verificar se a posição é a do personagem
        if (linha === x && coluna === y) {
          espacoMapa += "&";
        } else {
    ...
    // definir o mapa no elemento HTML
...
window.addEventListener("keydown", function testePersonagem(event) {
  const teclaPressionada = event.keyCode;
   ...
   if (teclaPressionada === 87) {
     ...
     if(boolNoClip == false){     
      ...
      if (mapaFaseDois[linha - 1][coluna] !== "<span class = 'parede'>*</span>" && mapaFaseDois[linha - 1][coluna] !== "D") {
  
        // verificação se há um "#" na posição anterior à posição atual do personagem na matriz
        if (mapaFaseDois[linha - 1][coluna] === "<span class = 'espinho'>#</span>") {
          linha = 6;
          coluna = 1;
          geracaoDoMapa();
          trocarVida();
  ...
  if (teclaPressionada === 68) {
    ...
    if (boolNoClip == false){
      ...
      // de <span class = 'parede'>*</span> e D
      if (mapaFaseDois[linha][coluna + 1] !== "<span class = 'parede'>*</span>" && mapaFaseDois[linha][coluna + 1] !== "D") {
        // verificação se há um "#" na posição anterior à posição atual do personagem na matriz
        if (mapaFaseDois[linha][coluna + 1] === "<span class = 'espinho'>#</span>") {
          linha = 6;
          coluna = 1;
          geracaoDoMapa();
          trocarVida();
  ...
  if (teclaPressionada === 73) {
        ...
        if (linha === 16 && coluna === 15) {
          ...
          mapaFaseDois[17][15] = "<span class = 'espinho'>#</span>"
